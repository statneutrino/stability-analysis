...
for (bio in unique(tms_processed$biomarker)){
    ...
    anova_fit <- rstatix::anova_test(data = df, dv = value, wid = sample_id, within = time)
    biomarker_name <- c(biomarker_name, bio)
    anova_results <- rbind(anova_results, rstatix::get_anova_table(anova_fit))
}
...
half_lives <- data.frame(
    ...
    sfo_hl_lower = log(2) / degrad_results$sfo_lower,
    sfo_hl_upper = log(2) / degrad_results$sfo_upper
) %>%
...
```{r conditiona_effect_plots, results='asis', echo=FALSE, warning=FALSE, message=FALSE}
for (bio in unique(tms_processed$biomarker)){
    df <- tms_processed %>% filter(biomarker == bio)
